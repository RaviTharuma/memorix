...
describe("package commands", () => {
	...
	let originalMemcodePackageDir: string | undefined;
	...
	beforeEach(() => {
		...
		originalMemcodePackageDir = process.env.MEMCODE_PACKAGE_DIR;
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".memorix"), { recursive: true });
		writeFileSync(join(projectDir, ".memorix", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".memorix", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".memorix", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("memcode install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "memcode --help" or "memcode install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: memcode install <source> [-l]");
...
`,
		...
		writeFileSync(
			join(projectDir, ".memorix", "settings.json"),
			JSON.stringify({ npmCommand: [originalExecPath, fakeNpmPath, "--prefix", projectPrefix] }, null, 2),
		);
		process.env.MEMCODE_PACKAGE_DIR = selfPackageDir;
