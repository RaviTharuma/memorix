...
Use `/logout` to clear credentials. Tokens are stored in `~/.memorix/agent/auth.json` and auto-refresh when expired.
...
Store credentials in `~/.memorix/agent/auth.json`:
