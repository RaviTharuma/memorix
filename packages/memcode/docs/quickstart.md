...
Uninstalling pi leaves settings, credentials, sessions, and installed pi packages in `~/.memorix/agent/`.
...
You can also run `/login` and select an API-key provider to store the key in `~/.memorix/agent/auth.json`.
