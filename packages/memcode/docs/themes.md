...
- Built-in: `dark`, `light`
- Global: `~/.memorix/agent/themes/*.json`
- Project: `.memorix/themes/*.json` (only after the project is trusted)
...
```bash
mkdir -p ~/.memorix/agent/themes
vim ~/.memorix/agent/themes/my-theme.json
```
