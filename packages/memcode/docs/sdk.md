...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.memorix/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.memorix/extensions/`)
- Project skills:
  - `.memorix/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.memorix/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.memorix/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.memorix/agent/auth.json and ~/.memorix/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.memorix/agent/extensions/`, `.memorix/extensions/`, and settings.json extension sources.
...
1. Global: `~/.memorix/agent/settings.json`
2. Project: `<cwd>/.memorix/settings.json`
