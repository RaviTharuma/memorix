...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @memorix/ai <command> [provider]
...
Examples:
  npx @memorix/ai login              # interactive provider selection
  npx @memorix/ai login anthropic    # login to specific provider
  npx @memorix/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @memorix/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @memorix/ai --help' for usage`);
	process.exit(1);
}
