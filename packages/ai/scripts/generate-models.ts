...
const MIN_GENERATED_MODEL_COUNT = 100;
...
export async function loadExistingGeneratedModelCount(): Promise<number> {
	try {
		const generatedUrl = pathToFileURL(join(packageRoot, "src/models.generated.ts")).href;
		const generated = (await import(`${generatedUrl}?count=${Date.now()}`)) as {
			MODELS?: Record<string, Record<string, Model<any>>>;
		};
		return countGeneratedModels(generated.MODELS ?? {});
	} catch {
		return 0;
	}
}

export function countGeneratedModels(providers: Record<string, Record<string, unknown>>): number {
	return Object.values(providers).reduce((total, models) => total + Object.keys(models).length, 0);
}

export function assertModelCatalogRefreshSafe(nextCount: number, existingCount: number): void {
	const minimum = Math.max(MIN_GENERATED_MODEL_COUNT, Math.floor(existingCount * 0.8));
	if (nextCount >= minimum || process.env[CATALOG_SHRINK_OVERRIDE_ENV] === "1") return;

	throw new Error(
		`Refusing to write shrunken model catalog: generated ${nextCount} models, existing catalog has ${existingCount}. ` +
			`This usually means a live catalog API partially failed. Re-run later, inspect the diff, or set ${CATALOG_SHRINK_OVERRIDE_ENV}=1 to override intentionally.`,
	);
}
...
`;
	...
	const generatedModelCount = countGeneratedModels(providers);
	const existingModelCount = await loadExistingGeneratedModelCount();
	assertModelCatalogRefreshSafe(generatedModelCount, existingModelCount);
